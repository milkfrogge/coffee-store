...
func TestService_GetAllProductsByCategory(t *testing.T) {
	...
	m.EXPECT().FindProductsByCategory(ctx, catId, uint32(0), uint32(0), "name").Return(ret, nil).Times(1)
...
func TestService_GetAllProductsByCategoryError(t *testing.T) {
	...
	for i := 0; i < len(catId); i++ {
		m.EXPECT().FindProductsByCategory(ctx, catId[i], uint32(0), uint32(0), "name").Return(ret[i], expErrors[i]).Times(1)

		uCase := NewService(m, log)

		prod, err := uCase.GetAllProductsByCategory(ctx, catId[i], uint32(0), uint32(0), 0)
